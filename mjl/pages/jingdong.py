...
conn=MySQLdb.connect(host='localhost',user='root',passwd='123456',db='mjl',charset='utf8')
cur=conn.cursor()
...
def get_mobile(n):
    ...
    res.encoding='utf-8'
    htmltext=etree.HTML(res.text)
    datas=htmltext.xpath('//li[contains(@class,"gl-item")]')
    mobile_list=[]
    for data in datas:   
        array_price=data.xpath('./div/div[3]/strong/i/text()[1]')
        price=",".join(map(str, array_price))
        array_intro=data.xpath('./div/div[4]/a/i/text()')
        intro=",".join(map(str, array_intro))
        array_ititle=data.xpath('./div/div[4]/a/em/text()')
        ititle=",".join(map(str, array_ititle))
        mobile_list.append(ititle)
        sql="insert into page_jingdong(ititle,price,intro) value('%s','%s','%s')"%(ititle,price,intro)
        cur.execute(sql)
        cur.close()
        conn.commit()
        conn.close()
